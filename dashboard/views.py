...
def dashboard_view(request):
    ...
    labels, entradas_por_dia, saidas_por_dia = [], [], []
    for i in range(dias):
        dia = inicio + timedelta(days=i)
        labels.append(dia.strftime("%d/%m"))
        soma_entradas = Conta.objects.filter(tipo='R', data_vencimento=dia).aggregate(total=Sum('valor'))['total'] or 0
        soma_saidas = Conta.objects.filter(tipo='P', data_vencimento=dia).aggregate(total=Sum('valor'))['total'] or 0
    ...
    top_categories = [
        {
            ...
            "total": float(item['total'] or 0)  # 👈 converte Decimal para float
        }
        for item in top_qs
    ]
    # === Dados para gráfico de pizza ===
    categorias_entrada = contas_periodo.filter(tipo='R').values('categoria__nome').annotate(total=Sum('valor')).order_by('-total')
    categorias_saida = contas_periodo.filter(tipo='P').values('categoria__nome').annotate(total=Sum('valor')).order_by('-total')

    categorias_entrada_labels = [c['categoria__nome'] or "Sem categoria" for c in categorias_entrada]
    categorias_entrada_values = [float(c['total']) for c in categorias_entrada]
    categorias_saida_labels = [c['categoria__nome'] or "Sem categoria" for c in categorias_saida]
    categorias_saida_values = [float(c['total']) for c in categorias_saida]
    ...
    context = {
        ...
        "categorias_entrada_labels": json.dumps(categorias_entrada_labels),
        "categorias_entrada_values": json.dumps(categorias_entrada_values),
        "categorias_saida_labels": json.dumps(categorias_saida_labels),
        "categorias_saida_values": json.dumps(categorias_saida_values),
        "dashboard": True,
    }
